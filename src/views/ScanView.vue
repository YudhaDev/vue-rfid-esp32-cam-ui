    ...
    >
      <div class="scan-flex">
        ...
        <div
          v-show="shimmer"
          id="loading-shimer"
          class="bg-slate-200 my-div animate-pulse mb-3"
        ></div>
        <canvas
          v-show="canvas"
          id="video-canvas"
...
import PopupScanRFIDMasuk from "../components/PopupScanRFIDMasuk.vue";
import { useStoreToggle } from "../stores/store";
import { useModalToggleScanMasuk } from "../stores/modal_state";
...
export default {
  setup() {
    ...
    return { storeToggle, modalToggleScanMasuk };
  },
  components: {
    // ScrollingText
    PersistentTextArea,
  ...
  methods: {
    handleClick() {
      this.modalToggleScanMasuk.toggleModal();
      this.shimmer = false;
      this.canvas = true;
  ...
  mounted() {
    ...
    this.socket.on("rfid-masuk", (data) => {
      this.modalToggleScanMasuk.rfidDetected();
    });
    this.socket.on("kehadiran-limit", (data) => {
      this.modalToggleScanMasuk.showOvertime();
    });
    this.socket.on("kehadiran-value", (data) => {
      this.modalToggleScanMasuk.kehadiranValueShow(data);
    });
    this.socket.on("welkam", (data) => {
        this.modalToggleScanMasuk.showWelcome(data)
    });
    this.socket.on("scan-masuk-sudah", (data) => {
        this.modalToggleScanMasuk.sudahScanHariIni()
    });
